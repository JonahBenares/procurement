...
class Reports extends CI_Controller {
	...
	public function pr_report(){
        ...
        $data['pr_no']=$this->super_model->select_custom_where('pr_head',"cancelled='0'");
    ...
    public function search_pr(){
        ...
        if(!empty($this->input->post('date_receive'))){
            ...
            $sql.=" ph.pr_date LIKE '%$date_receive%' AND";
            $filter .= "Date Received/Emailed - ".$date_receive.", ";
        }

        if(!empty($this->input->post('purpose'))){
            $purpose = $this->input->post('purpose');
            $sql.=" ph.purpose_id = '$purpose' AND";
        ...
        if(!empty($this->input->post('requestor'))){
            ...
            $sql.=" ph.requested_by = '$requestor' AND";
        ...
        if(!empty($this->input->post('description'))){
            ...
            $sql.=" pd.item_id = '$description' AND";
        ...
        $data['employees']=$this->super_model->select_all_order_by('employees',"employee_name",'ASC');
        $data['items']=$this->super_model->select_all_order_by('item',"item_name",'ASC');
        foreach($this->super_model->custom_query("SELECT ph.pr_id, ph.pr_no, ph.pr_date, ph.purpose_id, ph.enduse_id, ph.requested_by, pd.item_id, pd.quantity, pd.cancelled, pd.cancel_reason, pd.cancel_date, pd.pr_details_id, pd.add_remarks FROM pr_head ph INNER JOIN pr_details pd ON ph.pr_id = pd.pr_id WHERE ".$query) AS $head){
    ...
    public function export_pr(){
        ...
        if($date_received!='null'){
            $sql.=" ph.pr_date LIKE '%$date_received%' AND";
            $filter .= $date_received;
        }

        if($purpose!='null'){
            $sql.=" ph.purpose_id = '$purpose' AND";
            $filter .= $this->super_model->select_column_where('purpose', 'purpose_name', 'purpose_id', $purpose);
        }

        if($enduse!='null'){
            $sql.=" ph.enduse_id= '$enduse' AND";
            $filter .= $this->super_model->select_column_where('enduse', 'enduse_name', 'enduse_id', $enduse);
        }
            ...
            foreach($this->super_model->custom_query("SELECT ph.pr_id, ph.pr_no, ph.pr_date, ph.purpose_id, ph.enduse_id, ph.requested_by, pd.item_id, pd.quantity, pd.cancelled, pd.cancel_reason, pd.cancel_date, pd.pr_details_id, pd.add_remarks FROM pr_head ph INNER JOIN pr_details pd ON ph.pr_id = pd.pr_id WHERE ".$query) AS $head){
                ...
                if($po==1 && $partial==0){
                    ...
                    foreach($this->super_model->custom_query("SELECT pop.po_id, ph.po_date, pi.item_id FROM po_head ph INNER JOIN po_pr pop ON ph.po_id = pop.po_id INNER JOIN po_items pi ON pop.po_pr_id = pi.po_pr_id  WHERE pop.pr_id = '$head->pr_id' AND pi.item_id = '$head->item_id' AND ph.saved='1' AND ph.cancelled = '0' GROUP BY pi.item_id") AS $pod){
                        ...
                        $objPHPExcel->getActiveSheet()->getStyle('K'.$num)->getFont()->setColor($phpColor);
                        $status_remarks=date('m.d.y', strtotime($po_date)) . ' - Served DR#'. $this->super_model->select_column_where("dr_head",'dr_no','po_id',$po_id) . ' - Unserved: '.$balance;
                    }
                }else if($head->cancelled=='1'){
                    $status='Cancelled';
                    $status_remarks= $head->cancel_reason ." - " . date('m.d.y', strtotime($head->cancel_date));
                ...
                $unit_id = $this->super_model->select_column_where("item",'unit_id','item_id',$head->item_id);
                $unit = $this->super_model->select_column_where("unit",'unit_name','unit_id',$unit_id);
                $pr_date = date('m-d-y', strtotime($head->pr_date));
                $purpose=$this->super_model->select_column_where("purpose",'purpose_name','purpose_id',$head->purpose_id);
                $enduse=$this->super_model->select_column_where("enduse",'enduse_name','enduse_id',$head->enduse_id);
                $requestor=$this->super_model->select_column_where("employees",'employee_name','employee_id',$head->requested_by);
                $item_name=$this->super_model->select_column_where("item",'item_name','item_id',$head->item_id);
                $item_specs=$this->super_model->select_column_where("item",'item_specs','item_id',$head->item_id);
                $item = $item_name.", ".$item_specs;
                $styleArray = array(
                    'borders' => array(
                        'allborders' => array(
                          'style' => PHPExcel_Style_Border::BORDER_THIN
                ...
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$num, "$pr_date");
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$num, "$purpose");
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C'.$num, "$enduse");
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D'.$num, "$head->pr_no");
                ...
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.$num, "$head->quantity");
                ...
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M'.$num, "$head->add_remarks");
